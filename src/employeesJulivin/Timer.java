...
public class Timer {
    ...
    public Timer(String shopName, ArrayList<Person> persons) {
		...
		public void addEmployee() {
			...
			Person person = new Person(100, "Pedro",BigDecimal.valueOf(0),BigDecimal.valueOf(0),(0),BigDecimal.valueOf(0),"Monday",null,null,new ArrayList<String>());
		        persons.add(person);
		...
		public void setName(int newID, String name) {
			...
			System.out.println( "ID No.        name          DayOff     TTWPD ");
			for(Person p : persons) {
				System.out.println(String.format("%2d    %-15s  %15s   %8.1f", p.getId(),p.getName(),p.getDayOff(),p.getTotalTimeToWork()));
			}
		}
				public void listPersons() {
					convertTimeAtWorkToHour();
			System.out.println( "ID No.    name    DayOff     Absences   TimeAtWork(hours)   TTWPD        sdf     ");
			for(Person p : persons) {
				System.out.println(String.format("%2d    %-8s  %8s    %8d   %15.4f  %8.1f    %8.2f", p.getId(),p.getName(),p.getDayOff(),p.getAbsences(), p.getTimeAtWork(),p.getTotalTimeToWork(),p.getSdf()));
			}
		}
		...
		public void setTotalTimeToWorkPerDay(BigDecimal time) { 
			for (Person p : persons) {
		...
		Person p =findEmployeeByID(ID);
			...
			// p.getArriveLeave().add((date.toString()).substring(0,20));
			  if (((date.toString()).substring(0,2)).equalsIgnoreCase((p.getDayOff().substring(0,2)))){
                ...
                // p.getArriveLeave().add("No Work Today");
		...
		Person p =findEmployeeByID(ID);
		    ...
		    // p.getArriveLeave().add(date.toString());
		     p.getArriveLeave().add((date.toString()).substring(0,20));
		} 
}
		...
		public void serializePerson() {
			  try 
				 (FileOutputStream fos = new FileOutputStream("C:\\Users\\rvuy6\\Documents\\SerializePerson.txt");
		...
		public void deserializePerson() {
			...
			try (FileInputStream fis = new FileInputStream("C:\\Users\\rvuy6\\Documents\\SerializePerson.txt");
		...
		public void printAdminUse() {
			...
			System.out.println(" 105  -  To set the total time to work per day(ttwpd) for all employees.");
			System.out.println("");
